...
def get_datasest():
    with open("./datasets/bot_question.txt", 'r',encoding='utf-8', errors='ignore') as cf:
        docs = cf.readlines()
        print (len(docs))
    x_train = []
...
def train_doc2vec(x_train, size=200, epoch_num=1):
    ...
    model_dm.save('brain/hyper_bot_question.doc2vec')
    return model_dm
...
def doc2vec_sim_reply(str2,model):
    data = read_datas()
    sim_list = []
    sim_listx = []
    max_dis = 0.9
...
if __name__ == '__main__':
    ...
    doc2vec_sim_reply(str2,model1)
    #doc2vec_sim_reply(str2,model2)