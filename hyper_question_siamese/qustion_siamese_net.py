...
def siamese_train():
    # ------------------预加载------------------ #
    TRAIN_CSV = './data/hyper_bot_train_data.csv'
    savepath = './brain/hyperbot_question_siameselstm.h5'
    embedding_dict = {}
    # 读取并加载训练集
...
def return_highsim_sentence():
    # ------------------预加载------------------ #
    savepath = './brain/hyperbot_question_siameselstm.h5'
    embedding_dict = {}
    with open('./data/bot_cut_question.txt','r',encoding='utf-8') as data_file:
    ...
    sen2 = '高血压与失眠有关?吗？'
    dataframe = pd.DataFrame(
        {'question1': [data[i] for i in range(47592)], 'question2': [" ".join(jieba.lcut(sen2)) for i in range(47592)]})

    dataframe.to_csv("./data/predict.csv", index=False, sep=',', encoding='utf-8')
